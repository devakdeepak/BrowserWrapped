...
/// A right-to-left wipe transition: one large crescent-shaped wave,
/// tall enough to touch the top and bottom of the screen, sweeps from
/// the right edge to the left edge. Everything behind it (to its right)
...
class RippleCrescentTransitionRTL extends StatefulWidget {
  final VoidCallback onComplete;
  final Color fillColor;
  final Color waveColor;
  final Duration duration;

  const RippleCrescentTransitionRTL({
    super.key,
  ...
  @override
  State<RippleCrescentTransitionRTL> createState() => _RippleCrescentTransitionRTLState();
}

class _RippleCrescentTransitionRTLState extends State<RippleCrescentTransitionRTL>
  ...
  Widget build(BuildContext context) {
    return Positioned.fill(
      child: AnimatedBuilder(
        ...
        builder: (context, _) {
          return CustomPaint(
            painter: _CrescentWipePainterRTL(
...
class _CrescentWipePainterRTL extends CustomPainter {
  final double t; // 0..1, eased progress of the sweep
  final Color fillColor;
  final Color waveColor;

  _CrescentWipePainterRTL({
    required this.t,
  ...
  /// leftward, anchored at the top and bottom of the screen so it
  ...
  void paint(Canvas canvas, Size size) {
    // How far the crescent's belly pushes out to the left of its
    ...
    // The crescent travels from fully off the right edge to fully off
    // the left edge, sweeping right -> left.
    final travel = size.width + bulge * 2;
    final leadX = size.width + bulge - t * travel;
    final clampedLeadX = leadX.clamp(0.0, size.width);

    // ------------------------------------------------------------
    // Blackened region: everything already swept, to the right of
  ...
  @override
  bool shouldRepaint(covariant _CrescentWipePainterRTL oldDelegate) => oldDelegate.t != t;
}
